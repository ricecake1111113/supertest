...
<html lang="en-us">
  <head>
	...
	<link rel="preload" href="builds/5616/fonts/Matiz-webfont.woff" as="font" type="font/woff" crossorigin>
	<style>
		.container {
			width: 100%;
			height: 100%;
		...
		.closeButton {
			content:url("builds/5616/img/button_x.png?v=5616");
			position: absolute;
			width: 45px;
			height: 46px;
			top: -18px;
		...
		.closeButton:hover {
			content:url("builds/5616/img/button_x_hover.png?v=5616");
			width: 54px;
			height: 54px;
			top: -22px;
			right: -24px;
		...
		.largeWindow {
			width: 800px;
			height: 500px;
			background-image: url("builds/5616/img/troubleshoot_popup_bg.png?v=5616");
			margin-left: auto;
			margin-right: auto;
			margin-top: 100px;
			position: relative;
		...
		#importWindow {
			width: 600px;
			height: 400px;
			background-image: url("builds/5616/img/import_save_data_popup_bg.png?v=5616");
			text-align: center;
			margin-left: auto;
			margin-right: auto;
			margin-top: 100px;
		...
		#redeemWindow {
			width: 400px;
			height: 200px;
			background-image: url("builds/5616/img/redemption_code_popup_bg.png?v=5616");
			text-align: center;
			margin-left: auto;
			margin-right: auto;
			margin-top: 200px;
		...
		@font-face {
			font-family: 'matiz';
			src: url('builds/5616/fonts/Matiz-webfont.eot');
			src: url('builds/5616/fonts/Matiz-webfont.eot?#iefix') format('embedded-opentype'),
				 url('builds/5616/fonts/Matiz-webfont.woff') format('woff'),
				 url('builds/5616/fonts/Matiz-webfont.ttf') format('truetype'),
				 url('builds/5616/fonts/Matiz-webfont.svg#matizregular') format('svg');
			font-weight: normal;
			font-style: normal;
		}
		...
		#loadingCheck {
			...
			visibility: hidden;
			background-color: rgba(10, 139, 142, 0.85);
		}
	...
	<script>
		...
		var playsaurusPaymentsInitialized = false;

		var timer = {time: 0, running: false};
		...
		function addProduct(name, sku, imgUrl, priceInCents)
		{
			...
			unregisteredProducts.push( { name: name, sku: sku, image: imagePath, price: priceInCents } );
		}
		...
		function registerProducts()
		{
			...
			{
				registeredproducts = registeredproducts.concat(unregisteredProducts);
				// copy the products to a new array so we can clear out the old one
				var productsToRegister = [...unregisteredProducts];
				unregisteredProducts = [];
				playsaurusPayments.createPopup().then( 
					function() {
						initPlaysaurusPayments();
						playsaurusPayments.registerProducts(productsToRegister);
					}
				);
			}
		}
		
		function initPlaysaurusPayments()
		{
			if ( playsaurusPaymentsInitialized ) { return; }
			playsaurusPaymentsInitialized = true;
			...
			if (typeof playsaurusPayments['setPurchaseClosureCallback'] === 'function')
			{
				playsaurusPayments.setPurchaseClosureCallback(onPurchaseClosure);
			}
		...
		{
			timer.time = performance.now() - timer.time;
			timer.running = false;
		}
		...
		function startPurchase(sku, itemName)
		{
			...
			{
				if ( registeredproducts[i].sku == sku )
				{
					productIndex = i;
					break;
				}
		...
		function onPurchaseSuccess(purchaseData)
		{
			var product = purchaseData.product;
			var token = purchaseData.token;
			
			if ( token != null && token == purchaseToken )
			{
				sendToGame("PurchaseSuccessful", JSON.stringify(
					{
						sku: product.sku, 
		...
		function onPurchaseClosure(product)
		{
			pauseStopwatch();

			if(product != null && !timer.running)
			{
				console.log("closure - onPurchaseClosure with sku " + product.sku);
				sendToGame("PurchaseCanceled", JSON.stringify(
					{
						sku: product.sku, 
						price: product.price, 
						elapsedTimeInMs: timer.time
					}));
			}
			else
			{
				console.log("closure - onPurchaseClosure without a valid sku.");
			}
		}
		
	</script>
	<script onload="onPaymentAPILoaded()" src="https://payments.playsaurus.com/v1/main.js"></script>
	...
	<script>
		...
		{
			var notificationWindow = window.open("builds/5616/requestNotifications.html", "_blank", "width=500,height=300,toolbar=no,resizable=no,location=no,status=no,menubar=no,top=100,left=100");
		...
		function addNotification(title, message)
		{
			if ( areNotificationsEnabled() )
			{
				var notification = new Notification(title, { body: message, icon: "builds/5616/img/icon512x512.png" });
  ...
  <body style="text-align: center" onload="onPageLoaded()">
	...
	<img src="builds/5616/img/button_x.png?v=5616" style="display: none;"/>
	<img src="builds/5616/img/button_x_hover.png?v=5616" style="display: none;"/>
	<img src="builds/5616/img/import_save_data_popup_bg.png?v=5616" style="display: none;"/>
	<img src="builds/5616/img/redemption_code_popup_bg.png?v=5616" style="display: none;"/>
	...
	<div class="container" id="troubleshootUI">
		...
		<div class="largeWindow">
			<div class="largeWindowInfo">
				...
				<br/>
				<center>
				<img src="builds/5616/img/clear_cache.jpg"/>
				</center>
				<br/>
				...
				<br/>
				<center>
					<img src="builds/5616/img/local_storage_error.png"/>
				</center>
				<br/>
		...
		<div id="loadingParent">
			<div id="word">
				<center>
					...
					<br/>
					<span id="buildInfo">v1.0e12-5616</span>
					<br/>
					<br/>
					<div id="progressBar">
    ...
    <canvas id="unity-canvas" width=1280 height=720 style="width: 1280px; height: 720px; background: url('builds/5616/Build/5616.jpg') center / cover"></canvas>
	<script>
		updateClientLoadingProgress(0);
	</script>
    <script src="builds/5616/Build/5616.loader.js"></script>
	<script>
		var streamingAssetsDir = "builds/5616";
		...
		createUnityInstance(document.querySelector("#unity-canvas"), {
			dataUrl: "builds/5616/Build/5616.data.unityweb" + v,
			frameworkUrl: "builds/5616/Build/5616.framework.js.unityweb" + v,
			codeUrl: "builds/5616/Build/5616.wasm.unityweb" + v,
			streamingAssetsUrl: streamingAssetsDir + "/StreamingAssets",
			companyName: "Playsaurus",
			productName: "Clicker Heroes",
			productVersion: "1.0e12-5616",
